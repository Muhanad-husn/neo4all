"""
api/worker/jobs_agents_batch.py — ARQ jobs: batch agent curation pipeline (SPEC-07).

Batch-mode agent pipeline jobs
------------------------------
...
Cluster consolidation (_consolidate_clusters) merges overlapping pairwise
candidates into N-way cluster candidates before Agent-P, so it proposes
ONE merge per cluster instead of N conflicting pairwise merges.

See also: api/worker/jobs_agents.py for single-candidate jobs and shared
status helpers (AgentPipelineJobStatus).
"""
...
from api.worker.jobs_agents import (
    ...
    _get_agent_job_status,
    _set_agent_job_status,
)
