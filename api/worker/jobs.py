"""
api/worker/jobs.py — ARQ extraction job (SPEC-04 S-04.1).

Single job: extraction_job(ctx, run_id, chunk_id, doc_id)

Pipeline
--------
1. Operational idempotency check: if ChunkJobStatus.status == "complete" in
   Redis, log and return — prevents redundant LLM calls on re-enqueue. The
   structural idempotency guarantee (MERGE on dedupe keys) means re-running
   also cannot produce duplicate graph nodes or edges.
2. Mark status "running" in Redis (TTL 24 h).
3. Retrieve chunk text from Qdrant via VectorIndexer.get_chunk_text().
   Cache-aside: check CacheKey.chunk(chunk_id) first (30 min TTL per
   SKILL-D R-D8); populate on Qdrant hit. Fail closed if text absent.
4. Call ExtractionService.extract_chunk() — LLM call + schema validation.
   ExtractionError is raised on schema mismatch or malformed LLM output.
5. Write ExtractionResult via GraphWriter.write_extraction_result().
   GraphWriter uses MERGE on node_dedupe_key / rel_dedupe_key so re-running
   the same (run_id, chunk_id) always produces the same graph state.
6. Mark status "complete" with result counts.

Per-chunk failure handling
--------------------------
On ExtractionError or any unexpected exception:
  - Mark status "failed" with the error message.
  - Log ERROR with structured fields.
  - Return WITHOUT raising — other chunks in the same run are unaffected.

Job arguments (SKILL-A: primitive types only)
---------------------------------------------
  run_id:   Governed run identifier. Must have a locked schema in Redis.
  chunk_id: Target chunk (Chunk.chunk_id — 64-char SHA-256 hex).
  doc_id:   Parent document identifier; stored in ChunkJobStatus for monitoring.

Idempotency guarantees
----------------------
  Structural: GraphWriter.write_extraction_result() uses MERGE on
              node_dedupe_key / rel_dedupe_key — re-running the same
              (run_id, chunk_id) produces identical graph state.
  Operational: Status "complete" in Redis skips redundant LLM calls on
               re-enqueue. This is reset only by manual cache invalidation.

Sensitive data (SKILL-D R-D5)
------------------------------
  chunk.text is passed to the extraction service but never logged.
  Only chunk_id (a safe hash) appears in log entries.

ChunkJobStatus
--------------
  Persisted to Redis under CacheKey.job_status(run_id, chunk_id) with a
  24-hour TTL. Read by the monitoring endpoint GET /api/monitoring/jobs/{run_id}
  to surface per-chunk progress to the UI.
"""
